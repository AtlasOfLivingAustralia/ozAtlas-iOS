...
- (void)setButtonStyle:(JGActionSheetButtonStyle)buttonStyle forButtonAtIndex:(NSUInteger)index newTitle:(NSString *)newTitle{
    if (index < self.buttons.count) {
        UIButton *button = self.buttons[index];
        [button setTitle:newTitle forState:UIControlStateNormal]; // To set the title
        [self setButtonStyle:buttonStyle forButton:button];
    }
    else {
        NSLog(@"ERROR: Index out of bounds");
